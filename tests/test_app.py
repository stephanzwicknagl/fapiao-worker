...
def test_process_too_many_pdfs_rejected(client):
    files = [(io.BytesIO(b"%PDF-1"), f"invoice_{i}.pdf") for i in range(101)]
    r = client.post(
        "/process",
        data={
            "pdfs": files,
...
def _mock_fitz():
    ...
    def create_fake_doc():
        ...
        # Make save() handle both file paths and BytesIO objects
        def mock_save(path_or_buf):
            if isinstance(path_or_buf, str):
                # It's a file path - write to file
                MockPath(path_or_buf).write_bytes(b"%PDF-1.4 mock combined pdf content")
            else:
                # It's a file-like object (BytesIO) - write to it
                path_or_buf.write(b"%PDF-1.4 mock combined pdf content")

        fake_doc.save = mock_save
        return fake_doc

    return patch(
        "fapiao.web.fitz.open",
        side_effect=lambda *args, **kwargs: (
            create_fake_doc() if args and str(args[0]).endswith(".pdf") else create_fake_doc()
        ),
    )


@patch("fapiao.web.process_pdf_with_skipped")
@patch("fapiao.web._load_mappings")
def test_process_no_fapiaos_extracted_returns_error(mock_mappings, mock_process, client):
    mock_process.return_value = ([], [])  # (fapiaos, skipped_pages)
...
def test_process_truncates_over_40_fapiaos(mock_run2, mock_run1, mock_mappings, mock_process, client):
    fapiao = _fake_fapiao()
    mock_process.return_value = ([fapiao] * 50, [])  # (fapiaos, skipped_pages)
    mock_mappings.return_value = {fapiao["seller"]: "Groceries"}
...
def _make_pending_session(fapiaos, tmp_path):
    ...
    # Create a dummy combined.pdf file
    (pending / "combined.pdf").write_bytes(b"%PDF-1.4 dummy combined pdf")
...
@patch("fapiao.web.run2")
@patch("fapiao.web._save_new_mappings")
def test_categorize_valid_session_redirects_to_download(mock_save, mock_run2, mock_run1, client, tmp_path, monkeypatch):
    ...
    assert r.status_code == 302
    assert f"/download/{uuid}" in r.headers.get("Location", "")
    mock_save.assert_called_once()
    saved_mappings = mock_save.call_args[0][0]
    assert saved_mappings.get("沃尔玛（湖北）商业零售有限公司") == "Groceries"
    # Files should still exist (cleanup happens after downloads)
    assert (pending_dir / uuid).exists()


@patch("fapiao.web.run1")
@patch("fapiao.web.run2")
@patch("fapiao.web._save_new_mappings")
def test_categorize_preserves_pending_dir_for_download(mock_save, mock_run2, mock_run1, client, tmp_path, monkeypatch):
    """After categorize, files should remain for download endpoint."""
    fapiaos = [_fake_fapiao()]
    uuid, pending_dir = _make_pending_session(fapiaos, tmp_path)
    monkeypatch.setattr(app_module, "PENDING_DIR", pending_dir)
    ...
    # Files should still exist (cleanup happens after downloads)
    assert (pending_dir / uuid).exists()
    assert (pending_dir / uuid / "filled.xlsx").exists()
